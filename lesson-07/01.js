/*
В этом задании вам предстоит разработать интерактивный слайдер, который позволит
пользователю переключаться между созданными нейросетью изображениями веб-технологий.
...
- Обеспечьте циклическое переключение изображений: после последнего изображения
следует первое, и наоборот.
...
//текущий индекс(с 0, первое изображение)
let currentIndex = 0;
...
function handleNext() {
  // Увеличение индекса, если дошли до конца — переходим к 0
...
function handlePrev() {
  // Уменьшение индекса, если ушли в минус — переходим к последнему
...
// Явно устанавливаем первое изображение (для надёжности)
currentIndex = 0;
updateImage();