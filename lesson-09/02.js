...
Цель задания: Написать код для обработки событий нажатия на кнопки "Старт" и
"Отмена", реализуя обратный отсчёт с помощью JavaScript.
...
startButton.addEventListener('click', () => {
  ...
  // Сбрасываем счетчик на 3
  counter = 3
  // Отображаем 3 немедленно
  countdownDisplay.textContent = counter

  // Устанавливаем флаг, что таймер запущен
  isTimerStarted = true

  // Очищаем предыдущий интервал, если он был (на всякий случай)
  if (timerId) {
  ...
  timerId = setInterval(() => {
    ...
    if (counter === 0) {
      // Когда счетчик достигает 0, показываем ракету и останавливаем таймер
      countdownDisplay.textContent = '🚀'
      clearInterval(timerId)
      timerId = null
      isTimerStarted = false
    } else {
      // Иначе отображаем текущее значение счетчика
      countdownDisplay.textContent = counter
    }
  }, 1000)
})
