...
// Твой код:
// Добавляем обработчик события на контейнер petShop с использованием делегирования
petShop.addEventListener('click', function(event) {
  // Проверяем, что клик был по кнопке с питомцем (у нее есть класс 'pet')
  if (event.target.classList.contains('pet')) {
    // Проверяем, что в корзине меньше 3 питомцев
    if (cart.length < 3) {
      // Получаем id питомца из id элемента
      const petId = event.target.id;
      // Добавляем id в массив cart
      cart.push(petId);
      // Обновляем отображение корзины
      updateCartDisplay();
    } else {
